...
import { getBuilds, deleteBuildById } from 'apiSdk/builds';
import { BuildInterface } from 'interfaces/build';

type ColumnType = ColumnDef<BuildInterface, unknown>;
...
export function BuildListPage(props: BuildListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getBuilds({
        relations: ['user', 'report.count'],
        limit: params.pageSize,
        offset: params.pageNumber * params.pageSize,
        searchTerm: params.searchTerm,
        order: params.order,
        searchTermKeys: ['build_name.contains', 'build_status.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<BuildInterface>>(
    () => `/builds?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: BuildInterface) => {
    if (hasAccess('build', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/builds/view/${row.id}`);
    }
  };

  const columns: ColumnType[] = [
    { id: 'build_name', header: 'Build Name', accessorKey: 'build_name' },
    { id: 'build_status', header: 'Build Status', accessorKey: 'build_status' },
    {
      id: 'start_time',
      header: 'Start Time',
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/builds/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('build', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/builds/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('build', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="build" />
        </Flex>

        {hasAccess('build', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/builds/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
