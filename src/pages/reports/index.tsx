...
export function ReportListPage(props: ReportListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getReports({
        relations: ['test_run', 'build'],
        limit: params.pageSize,
        offset: params.pageNumber * params.pageSize,
        searchTerm: params.searchTerm,
        order: params.order,
        searchTermKeys: ['report_name.contains', 'report_status.contains'],
        ...(params.filters || {}),
      }),
  ...
  const columns: ColumnType[] = [
    { id: 'report_name', header: 'Report Name', accessorKey: 'report_name' },
    { id: 'report_status', header: 'Report Status', accessorKey: 'report_status' },
    hasAccess('test_run', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'test_run',
          header: 'Test Run',
          accessorKey: 'test_run',
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/test-runs/view/${record.test_run?.id}`}>
              {record.test_run?.test_name}
            </Link>
          ),
        }
      : null,
    hasAccess('build', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'build',
          header: 'Build',
          accessorKey: 'build',
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/builds/view/${record.build?.id}`}>
