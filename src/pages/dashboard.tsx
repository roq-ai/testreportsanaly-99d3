...
function DashboardPage() {
  ...
  const buildStat = useSWR<PaginatedInterface<BuildInterface>>(
    () => `/builds`,
    () => getBuilds({ limit: 1, offset: 0 }),
  );
  ...
  const testRunStat = useSWR<PaginatedInterface<TestRunInterface>>(
    () => `/test-runs`,
    () => getTestRuns({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Builds',
      result: buildStat?.data?.totalCount,
      isLoading: buildStat.isLoading,
    ...
    {
      title: 'Test Runs',
      result: testRunStat?.data?.totalCount,
      isLoading: testRunStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
