...
import { getTestRuns, deleteTestRunById } from 'apiSdk/test-runs';
import { TestRunInterface } from 'interfaces/test-run';

type ColumnType = ColumnDef<TestRunInterface, unknown>;

interface TestRunListPageProps {
...
export function TestRunListPage(props: TestRunListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getTestRuns({
        ...
        order: params.order,
        searchTermKeys: ['test_name.contains', 'status.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<TestRunInterface>>(
    () => `/test-runs?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: TestRunInterface) => {
    if (hasAccess('test_run', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/test-runs/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/test-runs/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('test_run', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/test-runs/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('test_run', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="test_run" />
        </Flex>

        {hasAccess('test_run', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/test-runs/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
