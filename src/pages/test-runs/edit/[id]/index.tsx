...
import { getTestRunById, updateTestRunById } from 'apiSdk/test-runs';
...
function TestRunEditPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<TestRunInterface>(
    () => (id ? `/test-runs/${id}` : null),
    () => getTestRunById(id),
  );
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: TestRunInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateTestRunById(id, values);
      mutate(updated);
      resetForm();
  ...
  const formik = useFormik<TestRunInterface>({
    initialValues: data,
    validationSchema: testRunValidationSchema,
